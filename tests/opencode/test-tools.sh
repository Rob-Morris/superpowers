...
if echo "$output" | grep -qi "superpowers-brain:brainstorming\|superpowers-brain:using-superpowers\|Available skills"; then
...
# Test 3: Test use_skill with superpowers-brain: prefix
echo ""
echo "Test 3: Testing use_skill with superpowers-brain: prefix..."
echo "  Running opencode with superpowers-brain:brainstorming skill..."

output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load superpowers-brain:brainstorming and tell me the first few lines of what you received." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "brainstorming\|Launching skill\|skill.*loaded"; then
    echo "  [PASS] use_skill loaded superpowers-brain:brainstorming skill"
else
    echo "  [FAIL] use_skill did not load superpowers-brain:brainstorming correctly"
    echo "  Output was:"
    echo "$output" | head -50
    exit 1
fi
