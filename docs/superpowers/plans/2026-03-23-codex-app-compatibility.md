...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers-brain:subagent-driven-development (recommended) or superpowers-brain:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
...
To:
```
- **superpowers-brain:using-git-worktrees** - REQUIRED: Ensures isolated workspace (creates one or verifies existing)
```
