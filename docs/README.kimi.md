# Superpowers Brain for Kimi Code

Complete guide for using the Superpowers Brain fork with [Kimi Code](https://github.com/MoonshotAI/kimi-code).
...
```text
/plugins install https://github.com/Rob-Morris/superpowers/tree/dev
```
...
Kimi Code reads Superpowers Brain skills from this repository. There are no copied skills, symlinks, hooks, or extra runtime dependencies.
...
Select Superpowers Brain and update it from there. Start a fresh session with `/new` after updating.

## Troubleshooting

### Plugin not loading

1. Run `/plugins info superpowers-brain` and check diagnostics.
...
Kimi Code installs the latest GitHub release for a bare repository URL when one exists. To test unreleased changes before the next Superpowers Brain release, install the branch explicitly:
...
1. Confirm `/plugins info superpowers-brain` shows the plugin enabled.
