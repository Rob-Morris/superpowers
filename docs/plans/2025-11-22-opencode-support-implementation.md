...
> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers-brain:executing-plans to implement this plan task-by-task.
 ...
 *
 * @param {string} skillName - Name like "superpowers-brain:brainstorming" or "my-skill"
 * @param {string} superpowersDir - Path to superpowers skills directory
 * @param {string} personalDir - Path to personal skills directory
 * @returns {{skillFile: string, sourceType: string, skillPath: string} | null}
 */
function resolveSkillPath(skillName, superpowersDir, personalDir) {
    // Strip superpowers-brain: prefix if present
    const forceSuperpowers = skillName.startsWith('superpowers-brain:');
    const actualSkillName = forceSuperpowers ? skillName.replace(/^superpowers-brain:/, '') : skillName;

    // Try personal skills first (unless explicitly superpowers-brain:)
    ...
    tools: [
      {
        ...
        schema: z.object({
          skill_name: z.string().describe('Name of the skill to load (e.g., "superpowers-brain:brainstorming" or "my-custom-skill")')
        }),
...
```javascript
      {
        ...
        execute: async () => {
          ...
          for (const skill of allSkills) {
            const namespace = skill.sourceType === 'personal' ? '' : 'superpowers-brain:';
...
**Skills naming:**
- Superpowers skills: \`superpowers-brain:skill-name\` (from ~/.config/opencode/superpowers/skills/)
...
**Below is the full content of your 'superpowers-brain:using-superpowers' skill - your introduction to using skills. For all other skills, use the 'use_skill' tool:**
...
```
use use_skill tool with skill_name: "superpowers-brain:brainstorming"
```
...
Run: `.codex/superpowers-codex use-skill superpowers-brain:brainstorming | head -20`
